...
# run函数
def run(env, side_name, Al, A_name, N):
    if not side_name:
        side_name = '红方'
            ...
            for k, v in enumerate(T_best_plan):
                if v >= 0:
                    for i, j in enumerate(T_number):
                        k = k - j
                        if k < 0:
                            T_weapon_obe[i].manual_attack(T_target_guid[v], T_weapon_id[i], 2)
                            last_target_weapon.append((T_target[v], T_weapon_obe[i]))
                            break
            end = timeit.default_timer()
        #    print('T第{}轮运行时间{}'.format(T_count, end - start))
...
#            print('L2第{}轮运行时间{}'.format(L2_count, end - start))
        ...
        if scenario.m_StartTime + duration <= scenario.m_Time:
            Time_list = [S1_time_list, S2_time_list, S3_time_list, M1_time_list, M2_time_list,
                         M3_time_list, L1_time_list, L2_time_list, T_time_list, [1-Des, count_hit/len(TARGET), count_hit, len(TARGET)]]
...
def main():
    ...
    if args.platform_mode == 'versus':
        ...
        run(env, args.side_name, A, AL_name[k], n)
    else:
        print('开发模式')
...
# AL = [ABC, ACO,IGA,ICA]
# AL_name = [ '威胁1_ABC', '威胁1_ACO','威胁1_IGA','威胁1_ICA']
AL = [RNN_WTA]
AL_name = ['RNN_20240621']
