...
class RNN_WTA(object):
    def __init__(self, num_weapon, num_target, Wei, Pij, Fij, Qjk, V_a):
        ...
        self.hidden_size = num_weapon * num_target
        self.batch_size = 1
        self.num_layers = 1
        self.iter_max = 30
        ...
        # RNN RnnModel
        self.decoder_net = RnnModel(self.input_size, self.hidden_size, self.batch_size, self.num_layers)
        self.decoder_optim = torch.optim.Adam(self.decoder_net.parameters(), lr=0.005)
    ...
    def train(self):
        for epoch in range(self.iter_max):
            ...
            outputs1 = torch.nn.functional.softmax(outputs, dim=1)
            # print(outputs_save.shape)
            # 使用期望作为损失
            loss1 = self.lossf(self.expect_1, self.expect_2, outputs1)
            # 使用打击概率作为损失
            # loss1 = self.lossf(self.hit_probability, outputs)
            # 使用威胁值作为损失
            # threat_value = self.threat_value.cpu()
            # threat_value = threat_value.numpy()
            # threat_value1 = np.tile(threat_value, (self.num_weapon, 1))
            # threat_value2 = torch.tensor(threat_value1).to(self.device)
            # loss1 = self.lossf(threat_value2, outputs)
            ...
            iter_mar1 = outputs1.clone().detach()
            iter_mar2 = iter_mar1.cpu()
            # iter_mar2 = torch.nn.functional.softmax(iter_mar2, dim=1)
            ...
            # 保存数据到csv文件
            if self.pointer2:
                with open('data_RNN_0.csv', 'a', newline='') as file:
