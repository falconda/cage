...
class GRO_WTA(object):
    def __init__(self, num_weapon, num_target, Wei, Pij, Fij, Qjk, V_a):
        ...
        self.rng = np.random.default_rng()  # 用于产生随机数
        self.MIN = min(num_weapon, num_target)   # 注意分三种情况讨论
        self.position = np.zeros((self.num_pop, self.MIN))
        self.position_new = np.zeros((self.num_pop, self.MIN))
        self.plan = np.full((self.num_pop, self.num_weapon), -1)
        self.plan_new = np.full((self.num_pop, self.num_weapon), -1)
    ...
    def rand_init_pos(self):
        ...
        for i in range(self.num_pop):
            ...
            self.position_new[i, :] = self.random_pos()
    ...
    def collaboration(self, i):
        # 随机选两个个体
        a = np.array(random.sample(range(0, self.num_pop), 2))
        digger1 = a[0]
        digger2 = a[1]
        r1 = self.rng.uniform(0, 1, self.MIN)
        pos1 = self.position[digger1, :]
        pos2 = self.position[digger2, :]
        D3 = pos2 - pos1
        position = self.position[i, :]
        position_new = position + r1 * D3
        return position_new

    def mining_method(self, i, l2):
        a = np.array(random.sample(range(0, self.num_pop), 2))
        digger1 = a[0]
        r1 = self.rng.uniform(0, 1, self.MIN)
        A2 = 2 * l2 * r1 - l2
        D2 = self.position[i, :] - self.position[digger1, :]
        position_new = self.position[digger1, :] + A2*D2
        return position_new

    def migartion_method(self, i, l1, best_position):
        r1 = self.rng.uniform(0, 1, self.MIN)
        r2 = self.rng.uniform(0, 1, self.MIN)
        C1 = 2 * r2
        A1 = 1 + l1 * (r1 - 1 / 2)
        D1 = C1 * best_position - self.position[i, :]
        position_new = self.position[i, :] + A1 * D1
        return position_new
    ...
    def search(self):
        for n in range(self.iter_max):
            ...
            for i in range(self.num_pop):
                plan1 = self.plan[i, :]
                plan2 = self.plan_new[i, :]
                pos1 = self.position[i, :]
                pos2 = self.position_new[i, :]
                fit1 = self.cal_single_fitness(plan1)
                fit2 = self.cal_single_fitness(plan2)
                if fit1 < fit2:
                    self.position[i, :] = pos2
                    self.plan[i, :] = plan2
                else:
                    self.position[i, :] = pos1
                    self.plan[i, :] = plan1
            # 更新当下pos，plan:降序
            ...
            # 更新算子
            sigma_initial = 2
            sigma_final = 1 / self.iter_max
            l2 = ((self.iter_max - self.t) / (self.iter_max - 1)) ** (2 * (sigma_initial - sigma_final)) + sigma_final
            l1 = ((self.iter_max - self.t) / (self.iter_max - 1)) ** (1 * (sigma_initial - sigma_final)) + sigma_final
            # 迁移、挖掘或协作 更新position_new
            rand = self.rng.uniform(0, 1, 1)
            if rand < 1/3:
                # 合作操作
                position_new = self.collaboration(i)
                plan_new = self.transformation(position_new)
                # 放入到表中
                self.position_new[i, :] = position_new
                self.plan_new[i, :] = plan_new
            elif 1/3< rand < 2/3:
                # 挖掘操作
                position_new = self.mining_method(i, l2)
                plan_new = self.transformation(position_new)
                # 放入到表中
                self.position_new[i, :] = position_new
                self.plan_new[i, :] = plan_new
                ...
                # 迁移操作
                position_new = self.migartion_method(i, l1, best_position)
                plan_new = self.transformation(position_new)
                # 放入到表中
                self.position_new[i, :] = position_new
                self.plan_new[i, :] = plan_new
            ...
            '''
            with open('data_GRO_0.csv', 'a', newline='') as file:
    ...
    def run(self):
        ...
        for a in range(self.num_pop):
            ...
            position_new = self.position_new[a, :]
            plan = self.transformation(position)
            plan_new = self.transformation(position_new)
            self.plan[a, :] = plan
            self.plan_new[a, :] = plan_new
        # 进行升序排序，最小的在前面
        self.position, self.plan = self.update(self.position, self.plan)
        self.position_new, self.plan_new = self.update(self.position_new, self.plan_new)
        # 算法群体更新
        self.search()
        # 选择最优的种马个体
