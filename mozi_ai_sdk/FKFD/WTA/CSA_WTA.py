...
class CSA_WTA(object):
    ...
    # search strategy
    def search_strategy(self, pos, best_pos, E):
        length = pos.shape[0]
        search_pos = pos
        for i in range(length):
            r2 = self.rng.random()
            if r2 < E:
                r1 = self.rng.integers(low=-1, high=1, size=1)
                h = self.rng.random()
                beta = 1 - (5 * self.t / self.iter_max) / 5
                theta = np.pi * (self.t / 10 * self.iter_max)
                search_pos[i] = best_pos[i] + r1 * np.cos(theta) * beta * h
        return search_pos

    def puncture_mechanism(self, pos, best_pos, F):
        length = pos.shape[0]
        puncture_pos = pos
        for i in range(length):
            r = self.rng.random()
    ...
    def search(self):
        ...
        for n in range(self.iter_max):
            ...
            for i in range(self.num_pop):
                # Single Crystal
                SelectPosition = self.position[i, :]
                SelectPlan = self.plan[i, :]
                SelectFit = self.fitness[i]
                # Main Crystal
                select = self.rng.integers(low=0, high=i+1, size=1)
                index = select[0]
                Crmain = self.position[index, :]
                # Random-selected Crystals
                RandNumber = self.rng.integers(low=0, high=i+1, size=1)
                RandSelectCrystal = self.rng.choice(i+1, size=RandNumber+1, replace=False)
                RandLength = len(RandSelectCrystal) # 确定随机选择个数
                RandCrystal = np.zeros((RandLength, self.MIN))
                for k in range(RandLength):
                    RandCrystal[k, :] = self.position[RandSelectCrystal[k], :]
                # Mean of randomly-selected Crystals
                Fc = np.mean(RandCrystal, axis=0)
                # Random numbers
                r = 2 * self.rng.random() - 1
                r1 = 2 * self.rng.random() - 1
                r2 = 2 * self.rng.random() - 1
                r3 = 2 * self.rng.random() - 1
                # New Crystals
                NewCrystal = np.zeros((4, self.MIN))
                NewCrystal[0, :] = self.position[i, :] + r * Crmain
                NewCrystal[1, :] = self.position[i, :] + r1 * Crmain + r2 * Crb
                NewCrystal[2, :] = self.position[i, :] + r1 * Crmain + r2 * Fc
                NewCrystal[3, :] = self.position[i, :] + r1 * Crmain + r2 * Crb + r3 * Fc
                # Evaluating New Crystals
                for j in range(4):
                    position = NewCrystal[j, :]
                    plan = self.transformation(position)
                    NewFit = self.cal_single_fitness(plan)
            ...
            # 保存数据到csv文件
            '''
            with open('data_CSA_0.csv', 'a', newline='') as file:
