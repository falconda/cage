...
class ProWTADataset(Dataset):
    ...
    def __init__(self, num_samples=1):
        ...
        # 按行遍历前num_samples行
        for index, row in data.iloc[2:num_samples+2].iterrows():
            # 提取蓝方的数据
    ...
    def init_red_input(self, num_target):
        self.weapon_type_wta = self.weapon_type[self.weapon_relation - 1]
        self.weapon_coordinates_wta = self.weapon_coordinates[self.weapon_relation - 1]
        self.weapon_damage_wta = self.weapon_damage[self.weapon_relation - 1]
        self.weapon_range_wta = self.weapon_range[self.weapon_relation - 1]
        self.weapon_ammunition_wta = self.weapon_ammunition[self.weapon_relation - 1]
        self.weapon_weight_wta = self.weapon_weight[self.weapon_relation - 1]
        # 红方信息处理
        # 武器类型
    ...
    def __getitem__(self, idx):
        ...
        return (self.dataset1[idx], [], self.dataset2[idx], self.dataset3[idx],
                self.Pij[idx], self.Threat[idx], self.Qjk[idx], self.norm_value, torch.tensor(self.num_weapon).to(device),
        torch.tensor(self.num_target).to(device))
