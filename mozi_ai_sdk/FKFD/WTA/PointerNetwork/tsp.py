...
def wta_update_mask(mask, dynamic, chosen_idx, num_weapon=20, num_target=20):
    ...
    end_c = torch.add(target_index, (num_weapon - 1) * num_target)
    for i in range(chosen_idx.size()[0]):
        target_array.append(list(range(target_index[i], end_c[i]+1, num_weapon)))
...
def reward(static, tour_indices):
    ...
    y = torch.cat((tour, tour[:, :1]), dim=1)# [256,21,2]
    # Euclidean distance between each consecutive point
    tour_len = torch.sqrt(torch.sum(torch.pow(y[:, :-1] - y[:, 1:], 2), dim=2))
    return tour_len.sum(1).detach()
def wta_reward(static, tour_indices):
    # Convert the indices back into a tour
    idx = tour_indices.unsqueeze(1)
    idx = idx.repeat(1, 2, 1)
    tour = torch.gather(static.data, 2, idx).permute(0, 2, 1)# [256,20,2]
    # Euclidean distance between each consecutive point
    tour_len = torch.prod(tour, 2)
    return torch.div(1, tour_len.sum(1)).detach()
