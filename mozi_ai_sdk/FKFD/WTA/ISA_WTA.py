        ...
        for n in range(self.iter_max):
            ...
            for i in range(self.num_pop):
                if i < self.num_pop - 1:
                    ...
                    if r1 <= self.a:
                        r3 = self.rng.random()
                        mirror_position = r3 * position + (1 - r3) * best_position
                        new_position = 2 * mirror_position - position
                    else:
                        new_position = self.random_pos()
                # 最优个体
                elif i == self.num_pop - 1:
                    rn = self.rng.normal(0, 1, size=self.MIN)
                    new_position = best_position + self.lamda * rn
                # 依据适应度选择个体位置
                new_plan = self.transformation(new_position)
            ...
            '''
            with open('data_ISA_0.csv', 'a', newline='') as file:
