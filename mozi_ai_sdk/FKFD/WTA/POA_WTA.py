...
class POA_WTA(object):
    ...
    def search(self):
        for n in range(self.iter_max):
            # 确定猎物位置和方案
            prey = self.rng.integers(self.num_pop)
            prey_position = self.position[prey, :]

            prey_fit = self.fitness[prey]
            # 探索阶段
            for i in range(self.num_pop):
                position = self.position[i, :]

                fitness = self.fitness[i]
                # 适应度：文章是寻找最小适应度，项目中是寻找最大适应度
                if prey_fit > fitness:
                    rand = self.rng.uniform(0, 1, self.MIN)
                    I = self.rng.integers(1, high=3)
                    p1_position = position + rand * (prey_position - I * position)
                else:
                    rand = self.rng.uniform(0, 1, self.MIN)
                    p1_position = position + rand * (position - prey_position)
                # 离散化
                p1_plan = self.transformation(p1_position)
                p1_fitness = self.cal_single_fitness(p1_plan)
                # 贪婪选择
                if p1_fitness > fitness:
                    self.position[i, :] = p1_position
                    self.plan[i, :] = p1_plan
                    self.fitness[i] = p1_fitness
            ...
            for j in range(self.num_pop):
                ...
                # 适应度：文章是寻找最小适应度，项目中是寻找最大适应度
                rand = self.rng.uniform(0, 1, self.MIN)
                p2_position = position + 0.2*(1 - self.t/self.iter_max)*(2*rand - 1)*position
                # 离散化
                p2_plan = self.transformation(p2_position)
                p2_fitness = self.cal_single_fitness(p2_plan)
                # 贪婪选择
                if p2_fitness > fitness:
                    self.position[j, :] = p2_position
                    self.plan[j, :] = p2_plan
                    self.fitness[j] = p2_fitness
            ...
            '''
            with open('data_POA_0.csv', 'a', newline='') as file:
