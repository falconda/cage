...
class RIME_WTA(object):
    ...
    def search(self):
        ...
        for n in range(self.iter_max):
            E = (self.t/self.iter_max)**(0.5)
            # 提出对于个体归一化适应度值
            _range = np.max(abs(self.fitness))
            norm_fit = self.fitness / _range    # 归一化到[-1, 1]
            ...
            for i in range(self.num_pop):
                ...
                search_pos = self.search_strategy(position, best_position, E)
                # hard-rime puncture mechanism
                F = norm_fit[i]
                puncture_pos = self.puncture_mechanism(search_pos, best_position, F)
                # positive greedy selection mechanism
                new_plan = self.transformation(puncture_pos)
                new_fit = self.cal_single_fitness(new_plan)
                if new_fit > fit:
                    self.position[i, :] = puncture_pos
                    self.plan[i, :] = new_plan
            ...
            '''
            with open('data_RIME_0.csv', 'a', newline='') as file:
