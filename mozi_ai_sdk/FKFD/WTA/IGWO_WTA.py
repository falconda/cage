...
class IGWO_WTA(object):
    def __init__(self, num_weapon, num_target, Wei, Pij, Fij, Qjk, V_a):
        ...
        self.t = 1
        self.elite = 50
        self.num_pop = 500   # 灰狼群体规模
        # self.iter_max = 300 # 分析收敛性使用
        self.iter_max = 1000
        self.alpha = 0.5
    ...
    def cal_fitness(self):
        ...
        # 取出最大适应度值
        # print('最大适应度值为：{}'.format(max(self.fitness)))
        # 保存当前轮
    ...
    def modular_position_update(self):
        # 剩下的delta狼：位置更新或进行算计方案：3头狼，30精英，17个delta狼
        for j in range(3+self.elite, self.num_pop):
    ...
    def local_search(self):
        # 对于精英个体：
        for j in range(3, 3+self.elite):
            rand = random.random()
            plan_base = self.plan[j, :]
            fitness_done = 0
            count1 = 0
            count2 = 0
            if rand <= self.p:
                ...
                if fitness_done > self.fitness[j]:
                    self.plan[j, :] = plan_base
    ...
    def search(self):
        if self.jump1 and self.jump2:
            ...
            for i in range(self.iter_max):
                ...
                # 保存数据到csv文件
                '''
                with open('data_IGWO_0.csv', 'a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([p1, p2])                
                '''

    def run(self):
        # print('feasibility is '.format(self.feasibility))
        self.rand_init_plan()
        self.search()
