...
import copy
import csv
sys.path.append('D:\\workplace\\moziai\\mozi_ai_sdk\\FKFD\\WTA\\DNN')
...
class LSTM_WTA(object):
    def __init__(self, num_weapon, num_target, Wei, Pij, Fij, Qjk, V_a):
        ...
        # LSTM LSTMModel
        self.decoder_net = LSTMModel(self.input_size, self.hidden_size,  self.num_layers, self.input_size)
    ...
    def train(self):
        for epoch in range(self.iter_max):
            ...
            # 保存数据到csv文件
            if self.pointer2:
                with open('data_LSTM_0.csv', 'a', newline='') as file:
